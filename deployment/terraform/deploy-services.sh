...
if [[ -z `printenv TF_VAR_nextcloud_endpoint` ]]; then
    echo "Stopping because TF_VAR_nextcloud_endpoint is undefined"
    exit 1
fi

if [[ -z `printenv TF_VAR_nextcloud_username` ]]; then
    echo "Stopping because TF_VAR_nextcloud_username is undefined"
    exit 1
fi

if [[ -z `printenv TF_VAR_nextcloud_password` ]]; then
    echo "Stopping because TF_VAR_nextcloud_password is undefined"
    exit 1
fi
